...
fn status_response(
    ...
    er_packet.par_read().for_each(|e| {
        if let C2SPackets::Status(C2SStatusPackets::Request(_)) = e.packet() {
            ew_packet.write(SendPacket::new(e.entity()).with_nochange(packet::s2c::status::response::S2CStatusResponsePacket::from(Status { // TODO: Readd 'prettier' status response event
