...
fn status_response(
    ...
    er_packet.par_read().for_each(|e| {
        if let packet::c2s::C2SPackets::Status(packet::c2s::status::C2SStatusPackets::Request(_)) = &e.packet {
            ew_packet.write(SendPacket::new(e.entity).with_before_switch(
                packet::s2c::status::response::S2CStatusResponsePacket::from(Status { // TODO: Readd 'prettier' status response event
